...
> 路径：`contribute/agnacos/` | 阿里 Nacos 双向集成：配置中心 + 服务发现
...
├── naming/          ← 服务发现：创建 INamingClient → 供 agkitex Resolver 使用
│
...
- **服务发现**（`naming/`）：创建 Nacos Naming Client，供 agkitex 的 `AgNacosResolver` 做服务端点发现
...
| 服务发现 | `nacos.naming` | `nacos.naming.enable`, `nacos.naming.serveraddr` |
...
| [[02-服务发现]] | NamingClient 创建、Kitex Resolver 消费 |
